...
class robotAI:
    ...
    def check_stopped(self, is_exact=False):
        if is_exact:
            return self.p3dx.last_pose == self.p3dx.pose
        else:
            return abs(self.p3dx.last_pose[0] - self.p3dx.pose[0]) < MARGIN_FOR_ERROR and \
                   abs(self.p3dx.last_pose[1] - self.p3dx.pose[1]) < MARGIN_FOR_ERROR
    ...
    def check_stuck(self):
        if self.check_stopped():
            if self.state == self.state_move_forward or self.state == self.state_move_backward:
                #print('> Robot possibly stuck at [' + str(self.p3dx.pose[0]) + ', ' +\
                #      str(self.p3dx.pose[1]) + ', ' + str(self.p3dx.pose[2]) + ']')
